...
/*
STM32F4内部内存分为:
1、普通内存SRAM(主要内存和辅助内存，地址从0x2000 0000开始，共128KB)，任何外设都可以访问
2、CCM内存(地址从0x1000 0000开始，共64KB)，这部分内存仅CPU可以访问，DMA之类的不可访问

内存管理控制器——mallco_dev结构体，同时实现对SRAM、CCM两个内存池的管理控制
