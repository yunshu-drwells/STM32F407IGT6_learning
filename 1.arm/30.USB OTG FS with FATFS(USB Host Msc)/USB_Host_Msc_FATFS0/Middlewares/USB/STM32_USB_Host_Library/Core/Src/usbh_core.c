...
USBH_StatusTypeDef  USBH_Process(USBH_HandleTypeDef *phost)
{
  ...
  if (phost->device.is_disconnected == 1U)
  {
	...
	//printf("USBH_Process->phost->gState:%d\n",phost->gState);
  switch (phost->gState)
  {
    case HOST_IDLE :
			//printf("State: HOST_IDLE\n");
			//printf("phost->device.is_connected:%d\n",phost->device.is_connected);
...
{
	//printf("USBH_LL_Connect->phost: %p\n", phost);
	//printf("Befer USBH_LL_Connect: phost->device.is_connected:%u\n", phost->device.is_connected);
