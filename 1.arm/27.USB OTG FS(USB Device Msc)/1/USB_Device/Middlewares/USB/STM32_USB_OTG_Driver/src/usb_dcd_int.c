  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1) /* In ITR */
    {
      ...
      if ( diepint.b.xfercompl )
      {
        ...
        USB_OTG_MODIFY_REG32(&pdev->regs.DREGS->DIEPEMPMSK, fifoemptymsk, 0);
        CLEAR_IN_EP_INTR_SELF(epnum, xfercompl);
        /* TX COMPLETE */
      ...
      if ( diepint.b.timeout )
      {
        CLEAR_IN_EP_INTR_SELF(epnum, timeout);
      }
      if (diepint.b.intktxfemp)
      {
        CLEAR_IN_EP_INTR_SELF(epnum, intktxfemp);
      }
      if (diepint.b.inepnakeff)
      {
        CLEAR_IN_EP_INTR_SELF(epnum, inepnakeff);
      }
      if ( diepint.b.epdisabled )
      {
        CLEAR_IN_EP_INTR_SELF(epnum, epdisabled);
      }       
      if (diepint.b.emptyintr)
      {
        ...
        CLEAR_IN_EP_INTR_SELF(epnum, emptyintr);
      }
    }
    epnum++;
    ep_intr >>= 1;
...
static uint32_t DCD_HandleOutEP_ISR(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  while ( ep_intr )
  {
    if (ep_intr&0x1)
    {
      ...
      {
        /* Clear the bit in DOEPINTn for this interrupt */
        CLEAR_OUT_EP_INTR_SELF(epnum, xfercompl);
        if (pdev->cfg.dma_enable == 1)
        {
      ...
      {
        /* Clear the bit in DOEPINTn for this interrupt */
        CLEAR_OUT_EP_INTR_SELF(epnum, epdisabled);
      }
      ...
      if ( doepint.b.setup )
      {
        ...
        CLEAR_OUT_EP_INTR_SELF(epnum, setup);
      }
    }
    epnum++;
    ep_intr >>= 1;
