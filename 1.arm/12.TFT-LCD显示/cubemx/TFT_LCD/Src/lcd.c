...
{ 		
	...
	typedef struct
{
  uint32_t AddressSetupTime;  //定义HCLK周期的数量来配置地址设置时间的持续期。设置与外部设备通信时的时序参数。该参数可以是 Min_Data = 0 和 Max_Data = 15 之间的值。
  uint32_t AddressHoldTime;  //定义HCLK周期的数量来配置地址保持时间的持续期。这通常与微控制器的内存接口配置相关，特别是在设置与外部设备通信的时序参数时使用。该参数可以是 Min_Data = 1 和 Max_Data = 15 之间的值。
  uint32_t DataSetupTime;  //定义HCLK周期的数量来配置数据设置时间的持续期。这通常与微控制器的内存接口配置相关，特别是在设置与外部设备通信的时序参数时使用。该参数可以是 Min_Data = 1 和 Max_Data = 255 之间的值。
  uint32_t BusTurnAroundDuration;  //定义HCLK周期的数量来配置总线周转时间的持续期。这通常与微控制器的内存接口配置相关，特别是在设置与外部设备通信的时序参数时使用。该参数可以是 Min_Data = 0 和 Max_Data = 15 之间的值。
  uint32_t CLKDivision;  //定义CLK时钟输出信号的周期，以HCLK周期的数量来表示。这通常与微控制器的时钟配置相关，特别是在设置时钟信号和它们的分频比例时使用。该参数可以是 Min_Data = 2 和 Max_Data = 16 之间的值。
  uint32_t DataLatency;  //定义要发出的内存时钟周期数
													 //在获取第一个数据之前先将其写入内存。
													 //参数值取决于内存类型，如下所示：
													 //- 如果是 CRAM，则必须设置为 0
													 //- 不关心异步 NOR、SRAM 或 ROM 访问
													 //- 在 NOR 闪存中，可能假定 Min_Data = 2 和 Max_Data = 17 之间的值
													 //具有同步突发模式使能
  uint32_t AccessMode;  //指定异步访问模式。该参数可以是@ref FSMC_Access_Mode 的值
	...
	*/
	...
	FSMC_NORSRAM_TimingTypeDef FSMC_ReadWriteTim;  //SRAM控制时序结构
	FSMC_NORSRAM_TimingTypeDef FSMC_WriteTim;  //SRAM扩展模式时序结构
   
	//①背光控制是GPIOH->PH9，在main.c->MX_GPIO_Init();中已经初始化完成了
	...
	//②SRAM句柄 在main.c->MX_FSMC_Init();中已经完成初始化
	...
	TFTSRAM_Handler.Init.ContinuousClock=FSMC_CONTINUOUS_CLOCK_SYNC_ASYNC;  //启用或禁用 FMC 时钟输出到外部存储设备。

	
	
  //③④ 在main.c->MX_GPIO_Init();中已经初始化完成了
	...
	*/
    ...
    if (lcddev.id == 0x7789 || lcddev.id == 0x9341 || lcddev.id == 0x1963)  /* 7789/9341/1963 提速 */
    {
		...
		lcd_display_dir(0);		//默认为竖屏
		LCD_BL(1);          // 点亮背光 背光是PH9
		printf("LCD Init down!\n");
		lcd_clear(WHITE);
}
/**
 * @brief       清屏函数
