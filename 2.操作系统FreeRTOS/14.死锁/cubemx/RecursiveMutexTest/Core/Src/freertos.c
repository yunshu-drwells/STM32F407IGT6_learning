...
void High_Task(void const * argument)
{
  ...
  for(;;)
  {
		printf("High task is waiting for Mutex!\n");
		if(pdPASS == xSemaphoreTake(PrMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
			printf("High task is running!\n");
		}
		if(pdPASS == xSemaphoreTake(PrMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
			printf("High task is running!\n");
		}
		printf("High task give back Mutex!\n");
		//释放二值信号量
...
void Low_Task(void const * argument)
{
  ...
  for(;;)
  {
    printf("Low task is waiting for Mutex!\n");
		if(pdPASS == xSemaphoreTake(PrMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
		...
		printf("Low task give back Mutex!\n");
		//释放二值信号量
		xSemaphoreGive(PrMutexHandle);
    osDelay(500);
  }
