...
void MX_FREERTOS_Init(void) {
  ...
  /* USER CODE END Init */
  /* Create the mutex(es) */
  /* definition and creation of PrMutex */
  osMutexDef(PrMutex);
  PrMutexHandle = osMutexCreate(osMutex(PrMutex));

  /* Create the recursive mutex(es) */
  /* definition and creation of myRecursiveMutex */
...
void High_Task(void const * argument)
{
  ...
  for(;;)
  {
		printf("High task is waiting for RecursiveMutex!\n");
		if(pdPASS == xSemaphoreTakeRecursive(myRecursiveMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
			printf("High task is running 1!\n");
		}
		if(pdPASS == xSemaphoreTakeRecursive(myRecursiveMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
			printf("High task is running 2!\n");
		}
		printf("High task give back RecursiveMutex 2!\n");
		//释放互斥信号量
		xSemaphoreGiveRecursive(myRecursiveMutexHandle);
		printf("High task give back RecursiveMutex 1!\n");
		//释放互斥信号量
		xSemaphoreGiveRecursive(myRecursiveMutexHandle);
    osDelay(500);
  }
...
void Low_Task(void const * argument)
{
  ...
  for(;;)
  {
    printf("Low task is waiting for RecursiveMutex!\n");
		if(pdPASS == xSemaphoreTakeRecursive(myRecursiveMutexHandle, portMAX_DELAY)){  //一直等待 获取互斥量
		...
		printf("Low task give back RecursiveMutex!\n");
		//释放二值信号量
		xSemaphoreGiveRecursive(myRecursiveMutexHandle);
    osDelay(500);
  }
