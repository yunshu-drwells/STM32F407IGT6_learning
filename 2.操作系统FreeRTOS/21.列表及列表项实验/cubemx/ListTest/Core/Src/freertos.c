...
static uint8_t record;  //记录列表插入个数
...
void Keyx_Task(void const * argument)
{
  ...
  /* Infinite loop */
	//初始化列表和列表项
	uint8_t index;
	vListInitialise(&ListTest);  //初始化列表项
	for(index=0; index<ITEM_NUMS; ++index){  //初始化列表
		vListInitialiseItem(&ListItemTest[index]);
		ListItemTest[index].xItemValue = index;
	}
	ListItem_t* pListItem;  //用于遍历列表
  for(;;)
  {
    //等待事件标志组置位
		...
		switch(x){
			case KEY0_EVENT_BIT:  //插入列表项
				if(record < ITEM_NUMS){  //小于列表项长度
